...
router.post('/register', registerValidator, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    console.debug('Register payload validation failed:', { body: req.body, errors: errors.array() });
  }
...
router.post('/login', loginValidator, async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    console.debug('Login payload validation failed:', { body: req.body, errors: errors.array() });
  }
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
...
router.post('/register-admin', registerValidator, async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    console.debug('Register-admin payload validation failed:', { body: req.body, errors: errors.array() });
  }
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
